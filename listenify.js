...
async function getSongInfo(ext) {
  const { url, id, qid, cid, bid, vid, pid, xs } = argsify(ext);

  // 若直接传入 URL 或 pid，直接返回
  ...
  const sources = [
    ...
    // 新增音源来源，调用第一个代码提供的逻辑
    { key: 'xs', url: xs, customHandler: async (data) => {
        const soundUrl = getCustomSoundUrl(data);
        return { urls: [soundUrl] };
      }
    }
  ];
  ...
  for (let source of sources) {
    if (source.url) {
      ...
      if (source.customHandler) {
        const result = await source.customHandler(data);
        if (result && result.urls && result.urls.length) {
          return jsonify({ urls: result.urls });
        }
      } else {
...
// 定义 getCustomSoundUrl，用于调用第一个代码中的音源逻辑
function getCustomSoundUrl(data) {
  // 这里假设 data 中包含 "soundUrl" 字段，请根据实际情况调整
  try {
    const parsed = JSON.parse(data);
    return parsed.soundUrl || null;
  } catch (e) {
    return null;
  }
}
